function ContractGrid(container, config) {
	var grid;
	var toolbar;
	var calendar;
	...
	this.setRowData = function(result) {
		console.log(result);
		setRowData(grid, result.id, result.data.form);
	};
	...
	this.init = function() {
		...
		grid.kidsXmlFile = config.recordsUrl;
	...
	function setEvent() {
		...
		toolbar.attachEvent("onClick", function(id) {
			...
			if (config.toolbar.callback.onClick)
		...
		grid.attachEvent("onOpenStart", function(id, m) {
			container.progressOn();
			return true;
		});
		grid.attachEvent("onOpenEnd", function(id, m) {
