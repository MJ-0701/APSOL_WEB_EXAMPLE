function FormDepartmentPopup(form, name, onSelected, onReload) {

	var params = '';
	var recordUrl = 'erp/department/records';
	var searchUrl = 'erp/department/search';
	...
	function onFormEvent() {
		...
		form.attachEvent("onKeyDown", function(inp, ev, _name, value) {
			...
			if (ev.keyCode == 13) {
				console.log(form.getItemValue(name));
				$.post(searchUrl, {
	...
	function setupGrid(pop) {
		...
		grid.load('xml/popup/department/grid.xml', function() {
