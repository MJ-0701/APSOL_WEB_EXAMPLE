function FormCustomerLevelPopup(form, name, onSelected, onReload) {

	var params = '';
	var recordUrl = 'popup/customer/level/records';
	var searchUrl = 'popup/customer/level/search';
	...
	function setupGrid(pop) {
		 ...
		 * gridMenu.loadStruct("xml/popup/customerLevel/gridMenu.xml");
		 * 
		 * gridMenu.attachEvent("onClick", function(menuitemId, type) { if
		 * (menuitemId == 'btnAdd') { pop.hide(); CustomerLevelWindow(); } });
		 */

		grid = pop.attachGrid(300, 300);
		grid.setImagePath(imageUrl);

		grid.load('xml/popup/customerLevel/grid.xml', function() {
