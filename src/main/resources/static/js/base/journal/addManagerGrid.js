function AddManagerGrid(container, config) {
	...
	function reload() {
		...
		/*
		 * container.progressOn(); var range = calendar.getRange();
		 * 
		 * var url = config.grid.record + "?from=" + range.from + "&to=" + range.to; grid.clearAndLoad(url, function() { try { grid.filterByAll(); } catch (e) { } container.progressOff(); }, 'json');
		 */
	...
	function setupToolbar() {
		...
		toolbar.attachEvent("onClick", function(id) {
			...
			case 'btnSearch':
				...
				break;
			case 'btnAdd':
				insertRowWithIndex(grid, "journalInfo/addManager", 'no', 0, index++, function(grid, id, data) {
				...
				dhtmlx.confirm({
					...
					callback : function(r) {
						if (r) {
							...
							$.post('journalInfo/deleteAddedManagerList', {
