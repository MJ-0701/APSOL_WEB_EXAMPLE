function BookGrid(container, config) {
	var grid;
	var toolbar;
	var dp;
	...
	this.init = function() {
		...
		grid.load(config.grid.xml, function() {
			...
			AccountingCell(grid, 3, function(rowId, cnt, data, byEnter) {
				...
				if (data == null) {
					...
					grid.cells(rowId, 2).setValue(data.uuid);
					grid.cells(rowId, 3).setValue(data.name);
