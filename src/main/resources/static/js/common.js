...
function alertFreeTier(){
	...
	dhtmlx.alert({
		title:"유료기능입니다.",
		type:"alert-error",
		text:"영업자(관리자)에게 문의해주세요.",
		callback:function(){
			// alertFreeTier();
		}
	});
	
	$(".dhtmlx_popup_button").hide();
...
function popupCustomerWindow(code, type, subCode) {
	
	if( code == '' || code == undefined)
		return;
	
	if(type != null && type != undefined && type != '')
		window.open('windowCustomer?code=' + code + '&type=' + type + '&subCode=' + subCode, "cst" + code, "resizable=0,width=1400,height=780");
	else
	window.open('windowCustomer?code=' + code, "cst" + code, "resizable=0,width=1400,height=780");
}

function popupCustomerWindowByBusinessNumber(businessNumber) {
	window.open('windowCustomer?bn=' + businessNumber, "cst" + businessNumber, "resizable=0,width=1400,height=780");
}
...
function insertData(grid, row, focusField, pos, fnOnAfter) {
	...
	if (hasTree) {
		var rowID = grid.getRowId(pos);
		if (rowID)
			grid.addRowBefore(row.id, dataArray, rowID);
		else
			grid.addRow(row.id, dataArray, pos);
	} else {
		grid.addRow(row.id, dataArray,pos);
	}
...
function setData(grid, rowId, field, value) {
	var colInd = grid.getColIndexById(field);
	if (colInd != undefined)
		grid.cells(rowId, colInd).setValue(value);
	else
		grid.setUserData(rowId, field, value);
}
...
function DateRangeCalendar(fromInput, toInput, onChangedRange) {
	 ...
	 * 어제
	 */
	this.setYesterDay = function() {
		me.setDateRange(getRangeYesterDay());
	};
	 ...
	 * 이번달 오늘까지
	 */
	this.setThisMonthToday = function() {
		me.setDateRange(getRangeThisMonthToday());
	};
...
function GridLoader(container, grid, config) {
	...
	this.reload = function(onLoaded) {
		...
		grid.load(url, function() {
			...
			if (config) {
				if (config.onAfterLoaded)
					config.onAfterLoaded();
			}
...
function getRangeYesterDay() {
	var fromDate = new Date();
	fromDate.setDate(fromDate.getDate() - 1);
	var defaultFromDate = fromDate.format("yyyy-MM-dd");
...
function getRangeThisMonthToday() {
	var fromDate = new Date();
	var defaultFromDate = fromDate.format("yyyy-MM-01");

	var defaultToDate = fromDate.format("yyyy-MM-dd");
...
function amount(amt, taxRate, type, scale, round) {
	...
	case VAT: {
		...
		var net = rounding(amt / rate, scale, round);
		var tax = rounding(amt - net, scale, round);
